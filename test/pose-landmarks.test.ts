...
describe('summarizeUpperBodyPose', () => {
  ...
  it('summarizes shoulders, hips, and torso geometry', () => {
    ...
    landmarks[12] = createLandmark(0.7, 0.5, 0.6);
    ...
    expect(summary.shoulderCenter).toEqual({ x: 0.5, y: 0.45, visibility: 0.7 });
    expect(summary.hipCenter).toEqual({ x: 0.5, y: 0.8, visibility: 0.8 });
    expect(summary.torsoCenter).toEqual({ x: 0.5, y: 0.625, visibility: 0.75 });
...
function createLandmark(x: number, y: number, visibility: number): NormalizedLandmark {
  return {
    x,
    y,
    z: 0,
