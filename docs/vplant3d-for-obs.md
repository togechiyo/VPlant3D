...
ハッカソン提出作業、投稿作業、デモ動画の最終収録は人間が担当する。

Codexは、VPlant3D for OBSの実装、動作確認、テスト、ドキュメント更新、必要な人力確認事項の記録に集中する。
...
- 16:9
- 透明背景でOBSへ重ねられる
- 右下または左下にVRMアバターを置きやすい
- 簡易配信部屋モードでは、1920x1080全体で見栄えが破綻しない
...
- OBS向けの実用ツールとして落ち着いたUI
- セットアップ画面は暗色ベースで視認性を優先
- アクセントカラーは状態表示、選択中、スライダー、ボタン、アウトライン、発光表現に使う
- 透明背景OBSモードではUIを出さず、アバターと3D要素の見え方を優先する
