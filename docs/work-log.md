...
## 2026-05-23 Stabilizer Hold Extension Fix

### Goal

- OBS側の欠測0ガードで追従性が落ち、数秒止まるように見える問題を直す

### Findings

- stabilizerが保持後の値を `previous` として扱っていたため、0フレームが連続すると保持期限後に再び「有効値から0への急落」と判定して保持を再開していた
- OBS Render側のhold時間800msも長く、欠測が続いた場合に固まったように見える原因になっていた

### Did

- stabilizerに `previousRaw` を追加し、連続0フレームは新しい急落として扱わないようにした
- OBS Render側のhold時間を800msから180msへ短縮した
- 連続0で保持が延長・再開され続けないユニットテストを追加した
...
- OBS実機で追従性が戻ったか確認する
- まだ0チラつきが出る場合はhold時間を少しだけ上げるより、欠測フレーム判定そのものにconfidence/sequence由来の情報を足す方向で進める
